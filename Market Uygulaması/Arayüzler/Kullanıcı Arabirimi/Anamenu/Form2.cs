...
using Barkod_Uygulaması.Arayüzler.Kullanıcı_Arabirimi.Ayarlar;
...
namespace Barkod_Uygulaması
{
    public partial class Form2 : Form
    {
        ...
        {
            string marketdi = Barkod_Uygulaması.Properties.Settings.Default.marketadi;
            int hiza =250;
            int althiza = 350;
            StringFormat st = new StringFormat();
            st.Alignment = StringAlignment.Near;
            e.Graphics.DrawString(""+marketdi+"", Baslik, sb, 350, 80, st);

            e.Graphics.DrawString("------------------------------------------------", altbaslik, sb, 250, 120, st);
            e.Graphics.DrawString("Ürün Adı             Fiyat              Adet", altbaslik, sb, 250, 150, st);
            e.Graphics.DrawString("------------------------------------------------", altbaslik, sb, 250, 180, st);

            for (int i = 0; i < listView1.Items.Count; i++)
            {
                e.Graphics.DrawString(listView1.Items[i].SubItems[1].Text, icerik, sb, 250, hiza, st);
                e.Graphics.DrawString(listView1.Items[i].SubItems[2].Text+ "₺", icerik, sb, 400, hiza, st);
                e.Graphics.DrawString(listView1.Items[i].SubItems[3].Text, icerik, sb, 550, hiza, st);
                hiza = hiza + 20;
                althiza = althiza + 20;
            }
            e.Graphics.DrawString("------------------------------------------------", altbaslik, sb, 250, althiza, st);
            althiza = althiza + 20;
            e.Graphics.DrawString("Toplam Tutar:      " + label2.Text + "₺", altbaslik, sb, 250, althiza, st);
            althiza = althiza + 20;
            e.Graphics.DrawString("------------------------------------------------", altbaslik, sb, 250, althiza, st);
        }
        ...
        {
            MessageBox.Show("Benimle İletişime Geçmek ve Geri Bildirimde Bulunmak İçin 'mehmetemirgonenc@gmail.com' mailine mail atabilirsiniz.Şimdiden Teşekkürler.", "İletişim", MessageBoxButtons.OK, MessageBoxIcon.Information);
        }
    }
}
